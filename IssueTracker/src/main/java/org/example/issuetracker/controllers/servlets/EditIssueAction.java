...
public class EditIssueAction extends AbstractServlet {
	...
	protected void performTask(HttpServletRequest request, HttpServletResponse response) throws ServletException,
		...
		if (stringIssueId == null || summary == null || description == null || statusString == null || type == null || priority == null || stringProject == null || stringBuild == null) {
		...
		if (!((statusNewString.equals(statusString) && stringAssignee == null) || (!statusNewString.equals(statusString) && stringAssignee != null))) {
		...
		if (!(((statusResolvedString.equals(statusString) || statusClosedString.equals(statusString))&& resolution != null) || (!statusResolvedString.equals(statusString) || !statusClosedString.equals(statusString)) && resolution == null)) {
		...
		int status;
		try {
			issueId = Long.valueOf(stringIssueId);
		...
		Issue issue = new Issue(issueId, summary, description, new Status(status), resolution, type, priority, new Project(projectId), new Build(buildId), new User(assigneeId), new User(modifiedBy));
		try {
